...
// Deterministic key generation from a seed (seed is arbitrary string; uses SHA-256 then first 32 bytes as seed)
export function generateKeyPairFromSeed(seed: string): KeyPair {
  const h = crypto.createHash('sha256').update(seed, 'utf8').digest();
  const seed32 = new Uint8Array(h.slice(0,32));
...
export function signFeed(feedSubset: unknown, secretKeyB64: string): string {
  ...
  if (key.length !== 64) throw new Error('SECRET_KEY must be 64 bytes (base64 of nacl secretKey)');
  const canonical = canonicalize(feedSubset);
  const sig = signDetached(Buffer.from(canonical), key);
...
export function verifyFeedSignature(feedSubset: unknown, signatureB64: string, publicKeyB64: string): boolean {
  ...
  return verifyDetached(Buffer.from(canonical), sig, pubKey);
}
