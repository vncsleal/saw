#!/usr/bin/env node
import { generateKeyPair, buildFeed, generateLlmsTxt } from './api.js';
import { verifyRemote, verifyLocal } from './verify.js';
import fs from 'node:fs';
import path from 'node:path';

// Resolve __dirname for ESM
// path resolution no longer needed in minimal CLI

// API key & detect functionality removed in minimal build.
...
function printHelp() {
  console.log([
    ...
    '  feed [--site <s>] [--out f] Build & sign feed (needs SAW_SECRET_KEY)',
    '  llms [--url <feedUrl>] [--public-key <base64>] [--out path] Generate llms.txt',
    '  verify <feed.json|site> [publicKeyBase64]  Verify feed signature',
    '  init                       Initialize sample block + keys + feed + llms.txt',
    ''
  ].join('\n'));
}
...
async function main() {
  ...
  // Group routing (feed|llms|key)
  if (['feed','llms','key'].includes(cmd)) {
    const group = cmd;
    const sub = arg2;
    const rest = tail;
    if (group === 'feed') {
  ...
  // antiscrape & detect commands removed
